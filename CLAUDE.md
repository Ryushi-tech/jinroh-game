...
### 2-0. シーンファイルの中身はチャットに出力しない（最優先ルール）

シーンファイル（`scene_*.txt`）を生成・バリデーション後は、**ファイル名のみ報告する**。

```
[生成完了] scene_day2_disc2.txt ✓
```

- シーンの発言内容をチャットに書いてはならない（verbatim であっても禁止）
- 理由: 私がチャットに書く瞬間に改変・省略・加筆が混入するリスクが生まれる
- ビューアーがシーンの正式な表示装置である。私はその役割を奪わない
- ユーザーが「〇行目を見せて」と指示した場合のみ、Read ツールの出力をそのまま貼ること（一切の編集禁止）
